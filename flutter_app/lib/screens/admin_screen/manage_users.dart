...
class UserManagementsPage extends StatefulWidget {
  const UserManagementsPage({Key? key}) : super(key: key);

  @override
  _UserManagementsPageState createState() => _UserManagementsPageState();
}

class _UserManagementsPageState extends State<UserManagementsPage> {
  final UsersManager _usersManager = UsersManager(apiService: ApiService(baseUrl: baseURL));
  List<Admin> _admins = [];
  bool _isLoading = true;
  List<Admin> _filteredAdmins = []; // Liste filtrée
  ...
  Future<void> _loadUsers() async {
    ...
    try {
      final users = await _usersManager.getUserManagements();
      setState(() {
        _admins = users;
      ...
      showCustomDialog(context, "$e", DialogType.error);
      setState(() {
        _isLoading = false;
      });
    }
                ...
                child: _isLoading
                  ...
                  child: Text(
                    _searchController.text.isNotEmpty
                        ? appLocalizations.noUserManagementFound
                        : appLocalizations.noUserManagementFound,
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              _buildInfoRow(Icons.shield, appLocalizations.userLevel, appLocalizations.userManagementLevel(admin.userLevel ?? 2), theme, appLocalizations), // ← ICI
              _buildInfoRow(
                Icons.circle,
  ...
  Future<void> _deleteUser(User user, ThemeData theme, bool isDarkMode, AppLocalizations appLocalizations) async {
    ...
    if (confirmed == true) {
      try {
        ...
        showCustomDialog(context, appLocalizations.userDeletedSuccess, DialogType.success);
        _loadUsers();
      } catch (e) {
        showCustomDialog(context, appLocalizations.errorDeletingUser, DialogType.error);
      }
    }
  }
}